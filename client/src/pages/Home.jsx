    ...
    <div className="h-full flex flex-col overflow-y-auto overflow-x-hidden pb-10 md:pr-2 space-y-16 md:space-y-20 bg-dark-bg">
            ...
            >
              ...
              <motion.div variants={fadeIn} className="pt-8 flex items-center gap-4 text-sm text-gray-500 font-medium">
                ...
                <div>
                  ...
                  <p>Trusted by <span className="text-dark-text">10,000+</span> students</p>
                </div>
              </motion.div>
            </motion.div>
          </div>
      ...
      <section className="max-w-[1400px] mx-auto w-full px-4 md:px-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 relative">
          ...
          {[
            { icon: Users, label: 'Active Students', value: `${globalStats.activeStudents > 10000 ? globalStats.activeStudents : "10,000+"}` },
            { icon: BookOpen, label: 'Subject Notes', value: `${globalStats.subjectNotes > 0 ? globalStats.subjectNotes + "+" : "500+"}` },
            { icon: PlayCircle, label: 'Video Lectures', value: `${globalStats.videoLectures > 0 ? globalStats.videoLectures + "+" : "1,200+"}` },
      ...
      <section className="max-w-[1400px] mx-auto w-full px-4 md:px-8 relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {(settings?.features?.length > 0 ? settings.features : [
            ...
            >
              {/* Hover Glow */}
              <div className={`absolute -inset-px rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-sm pointer-events-none ${
                i === 0 ? 'bg-gradient-to-b from-blue-500/20 to-transparent' : 
      ...
      <section className="max-w-[1400px] mx-auto w-full px-4 md:px-8 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
          {(settings?.testimonials?.length > 0 ? settings.testimonials : [
            { name: 'Rahul Sharma', role: 'B.Tech CS, 3rd Year', text: 'NoteVault Pro completely changed my study routine. The organized notes saved me hours of searching for materials before finals.' },
      ...
      <section className="max-w-[1400px] mx-auto w-full px-4 md:px-8 py-10">
        <div className="relative rounded-[30px] md:rounded-[40px] overflow-hidden bg-gradient-to-r from-primary/90 to-purple-600 p-8 sm:p-12 md:p-20 text-center">
           ...
           <div className="relative z-10 max-w-3xl mx-auto space-y-6 md:space-y-8">
             ...
             <p className="text-base sm:text-xl text-white/80">Join 10,000+ students who are already learning smarter and scoring higher with NoteVault Pro.</p>
             <button 
      ...
      <footer className="mt-16 md:mt-20 border-t border-dark-border bg-dark-bg pt-16 md:pt-24 pb-12 relative">
        <div className="max-w-[1400px] mx-auto px-6 md:px-16">
            ...
            <div className="lg:col-span-2 space-y-8">
              <div className="flex items-center gap-3">
                ...
                <span className="text-3xl font-extrabold text-dark-text tracking-wide">
                  {settings?.siteName ? settings.siteName.replace(' Pro', '') : 'NoteVault'}<span className="text-primary font-medium">{settings?.siteName?.includes(' Pro') ? 'Pro' : ''}</span>
                </span>
              </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between pt-8 border-t border-dark-border text-sm text-dark-muted font-medium">
            ...
            <p>© {new Date().getFullYear()} {settings?.siteName || 'NoteVault Pro'}. All rights reserved.</p>
          </div>
        </div>
      </footer>
    </div>
